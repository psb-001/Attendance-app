-- ==========================================
-- THE MASTER CLEAN SLATE DATABASE REBUILD
-- ==========================================

-- 1. TURN OFF THE SYNC ENGINE TO AVOID ERRORS DURING PURGE
...
-- 2. PURGE OVERLAPPING/CONFLICTING TABLES
-- Warning: This deletes these tables completely to rebuild them cleanly
...
-- ==========================================
-- CORE REGISTRIES (SOURCES OF TRUTH)
-- ==========================================

-- 3. BUILD STUDENTS REGISTRY
...
-- Seed default subjects so Teacher Dashboard has data 
...
CREATE TABLE public.profiles (
    ...
    email TEXT UNIQUE NOT NULL,
    full_name TEXT,
    role TEXT DEFAULT 'not_enrolled',
    branch TEXT,
    roll_no INTEGER,
    department TEXT,
    subjects JSONB,
...
-- ==========================================
-- THE NEW IDENTITY SYNC ENGINE
-- ==========================================

-- 10. CREATE SMART SYNC FUNCTION
...
-- ==========================================
-- BULLETPROOF SECURITY ROLES (RLS)
-- ==========================================
...
-- 13. SET POLICIES (Simple & Unbreakable for Reads)
...
-- We also need simple policies to allow writes from clients for app functionality:
-- * Teachers handle these writes via the App itself
CREATE POLICY "Allow all actions for authenticated" ON public.profiles FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Allow all actions for authenticated" ON public.students_registry FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Allow all actions for authenticated" ON public.teachers FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Allow all actions for authenticated" ON public.subjects FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Allow all actions for authenticated" ON public.attendance_logs FOR ALL USING (auth.role() = 'authenticated');

-- ==========================================
-- FINAL ACTION: REFRESH ALL EXISTING USERS
-- ==========================================
DO $$
DECLARE
    user_record RECORD;
